...
- Use **list** or **glob** to discover paths before reading or editing. For a workspace overview, call **list** with \`recursive: true\` and optional \`maxDepth\` (limit levels below \`path\`) and \`directoriesOnly: true\` (folders only).
...
export function prompt(options?: FileSystemPromptOptions): string {
  ...
  return [FILE_SYSTEM_PROMPT_BASE, formatPermissionsForPrompt(permissions)].join(
    "\n\n",
  );
}

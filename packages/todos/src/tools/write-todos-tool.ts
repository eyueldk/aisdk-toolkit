...
For extended examples (positive and negative), use a longer custom description if you need them.
...
/**
 * AI SDK tool `writeTodos`: each call replaces the entire todo list.
 */
export function createWriteTodosTool(options: CreateWriteTodosToolOptions) {
  return tool({
